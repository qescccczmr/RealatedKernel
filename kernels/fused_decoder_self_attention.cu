...
__global__ void masked_MHA_kernel(T* q,
    ...
    // ( ) note: below one line is wrong kv head id access way.
    ...
    if (tid * vec_size < head_size) {
        ...
        // ( ) note: will enable below code lines when qkv bias is available
        // if (qkv_bias != nullptr){
    ...
    // ( ) note: define dynamic smem type is char type!! not T
    ...
    for(int iter = 0; iter < step; iter++) {
        // ( ) note: every iter,  q and k vector's shape = [1, head size]
        // reuse k cache
        ...
        // ( ) note: when final step, update k cache and fetch k from input k vec, rather than kv cache
...
void launchDecoderMaskedMHA(TensorWrapper<T>* qkv_buf,
    ...
    // ( )note: we should carefully get shape from tensorwrapper, DON'T be GOT wrong value
