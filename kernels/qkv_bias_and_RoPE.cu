...
//input:QKV [nmstokens,qkvheadnum,headsize]
//output:q:[bs,qheadnum,maxqlen,headsize]
//       k:[bs,kvheadnum,maxqlen,headsize]
//       v:[bs,kvheadnum,maxqlen,headsize]
...
{
    // ( )note: in Llama, rotate size = 64, we are not able to vectorizedly read data.
    ...
    int dst_token_id = token_id + token_padding_offset; // token id after rebuild padding
     //batch1 1(1 1 1 1) 2(1 1 1 1) 3(1 1 1 1) 4(1 1 1 1) seq_len=4
     //batch2 5(1 1 1 1) 6(1 1 1 1) 7(1 1 1 1) 8(1 1 1 1)
    int batch_id = dst_token_id / seq_len;       // 第几个seq  seqlen is max_seq_len for padding used to unify all seq's length
    int local_token_id = dst_token_id % seq_len; // seqlen维度里面具体哪个token  每个seq中的局部token id
    // 2. bias add
    //定位到线程处理的哪个位置数据
